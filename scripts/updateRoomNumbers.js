...
/**
 * Script to update existing room numbers to sequential format (01, 02, 03, etc.)
 * Run this to fix room numbering for existing hostels
 */
const updateRoomNumbers = async () => {
  try {
    ...
    // Find John Hostel (or update to find any hostel by name)
    ...
    // Get all rooms for this hostel, sorted by current room number
    ...
    // Update each room with new sequential numbering
    const bulkOps = [];
    for (let i = 0; i < rooms.length; i++) {
      const newRoomNumber = (i + 1).toString().padStart(2, '0');
      const floor = Math.floor(i / 15) + 1; // 15 rooms per floor
