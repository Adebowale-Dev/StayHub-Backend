...
async function testPaystackConnection() {
  ...
  try {
    ...
    // Test API connection - List transactions
    ...
    try {
      const response = await axios.get(
        'https://api.paystack.co/transaction',
        {
          ...
          params: {
            perPage: 1 // Just fetch 1 transaction to test connection
          }
        }
      );
    ...
    // Test transaction verification with a dummy reference
    console.log('\n3️⃣ Testing transaction verification endpoint...');
    
    try {
      await axios.get(
        'https://api.paystack.co/transaction/verify/DUMMY_REF_123',
        {
          headers: {
            'Authorization': `Bearer ${secretKey}`
          }
